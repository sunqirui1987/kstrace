...
const (
	podNotFoundError              = "no pod found "
	podPhaseNotAcceptedError      = "cannot attach into a container  current phase is %s"
...
func (a *Attacher) AttachJob(traceJobID types.UID, namespace string) {
	a.Attach(fmt.Sprintf("%s=%s", TraceIDLabelKey, traceJobID), namespace)
}
